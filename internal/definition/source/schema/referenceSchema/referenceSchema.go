...
type referenceImp struct {
	...
	ref        string
	resolving  bool
	resolution headers.Schema
}

var _ headers.Resolvable = (*referenceImp)(nil)

func New(title string, data headers.Raw) headers.Schema {
...
func (imp *referenceImp) Resolve(openAPI headers.OpenAPI) headers.Schema {
	...
	if res, ok := target.(headers.Resolvable); ok {
